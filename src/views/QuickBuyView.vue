...
const productIdToBuy = route.params.productIdToBuy;
const quantityToBuy = route.query.quantity;

const {data: cartData, isPending, isError} = useQuery({
  queryKey: ['cart'],
  queryFn: async () => {
    const {data} = await axios.get(`/products/${productIdToBuy}?client=true`);
      ...
      result: [
        {
          ...
          productPrice: data.result.price,
          productThumbnail: data.result.thumbnail,
          productPromotionPrice: data.result.promotionPrice,
          productCategoryId: data.result.categoryId,
          productCategoryName: data.result.categoryName
        }
      ]
    };
  },
