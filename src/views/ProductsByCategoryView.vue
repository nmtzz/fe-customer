...
const {data: productsData, isLoading: isLoadingProducts, isError} = useQuery({
  queryKey: ['products', locale.value, route.params.categoryId],
  queryFn: async () => {
    try {
  ...
  enabled: computed(() => !!route.params.categoryId)
});
const {data: categoryData} = useQuery({
  queryKey: ['category', categoryId.value],
  queryFn: async () => {
    try {
      const {data} = await axios.get(`/categories/${categoryId.value}?client=true`);
      return data;
    } catch (error) {
  ...
  enabled: computed(() => !!route.params.categoryId),
  select: (data) => {
    if (data && data.result && !data.result.status) window.location.href = '/404';
    return data;
  }
})

const priceRangeExtents = computed(() => {
  if (!productsData.value || !productsData.value.result) return [0, 10000];

  const products = productsData.value.result.filter(product =>
      product.isActive && product.categoryId === categoryId.value
  );
...
const filteredProducts = computed(() => {
  ...
  let products = productsData.value.result.filter(product =>
      product.categoryStatus && product.status && product.categoryId === categoryId.value
  );
...
const categoryName = computed(() => {
  document.title = categoryData.value?.result?.name || t('category.title', 'Products');
  return categoryData.value?.result?.name || '';
});
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-white">
              {{ categoryName || t('category.title', 'Products') }}
            </h1>
          </div>
          <button
              @click="toggleFilters"
        ...
        <nav class="flex py-3" aria-label="Breadcrumb">
          <ol class="flex items-center space-x-1">
            ...
            <li class="flex items-center">
              ...
              <span class="ml-1 text-primary-600 text-sm font-medium" aria-current="page">{{ categoryName }}</span>
            </li>
          </ol>
        </nav>
      </div>
          ...
          <div class="bg-white border rounded-lg p-4 sticky top-4">
            ...
            <div class="mb-6">
              ...
              <div v-else class="text-sm text-gray-500 italic">
                {{ t('category.noBrands', 'No brands available for this category') }}
              </div>
        ...
        <div class="flex-grow">
          <div class="flex justify-between items-center mb-4">
            <p class="text-sm text-gray-500">
              {{ filteredProducts.length }} {{ t('category.productsFound', 'products found') }}
            </p>
          </div>
          ...
          <div v-else-if="isError" class="py-8 text-center">
            <p class="text-red-500">{{ t('category.errorLoading', 'Error loading products') }}</p>
          </div>
          ...
          <div v-else-if="filteredProducts.length === 0" class="py-8 text-center bg-gray-50 rounded-lg">
            ...
            <h3 class="mt-2 text-lg font-medium text-gray-900">{{ t('category.noProducts', 'No products found') }}</h3>
            <p class="mt-1 text-gray-500">
              {{ t('category.tryDifferentFilters', 'Try different filters or check back later') }}</p>
            <button
                @click="clearFilters"
          ...
          <div v-else>
            ...
            <div class="mt-8 text-center">
              ...
              >
                {{ t('category.loadMore', 'Load More Products') }}
              </button>
              <p
                  v-else-if="filteredProducts.length > 0"
                  class="text-gray-500 py-3"
              >
                {{ t('category.allProductsLoaded', 'All products have been loaded') }}
              </p>
            </div>
          </div>
        </div>
